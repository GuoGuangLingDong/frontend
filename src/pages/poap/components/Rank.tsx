...
import { useGoDetails } from "./NewList";
...
export const Share = ({ amount, ...props }: { amount: number } & IDIVProps) => {
  return (
...
export const RankItem = ({ item, index }: { item: any, index: number } & IDIVProps) => {
  ...
  return (<div className="rectangle relative h-64 p-4 pt-10 md:p-10" style={{
    ...
    <div className="text-xs flex items-center" style={{ color: secondColor }}>
      <div className="flex-1"><Share amount={item.holder_num} /></div>
...
export const RankList = ({ data }: { data: IPoap[] }) => {
  return (<BodyBox>
    ...
    <ScrollBox data={data} rowCount={5}>
      <div className="flex justify-center m-auto" style={{ width: 200 * data?.length }}>
        {data?.map((item, i) => {
